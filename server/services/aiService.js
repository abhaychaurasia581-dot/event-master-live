...
const aiService = {
  ...
  async recommendEvents(userId) {
    ...
    try {
      ...
      const upcomingEventsResponse = await eventModel.getEvents({ limit: 50 });
      ...
      const fallbackResponse = await eventModel.getEvents({ limit: 10 });
  ...
  async getTrendingEvents(limit = 10) {
    try {
      const response = await eventModel.getEvents({ limit });
